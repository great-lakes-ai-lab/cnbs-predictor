...
Scope: Temperature path only. Evaporation and Precipitation paths follow
...
def _build_fake_flxf_dataset(mean2t_value=273):
    """
    Build a fake xarray Dataset mimicking what ``cfgrib.open_dataset`` returns
    for a CFSv2 flxf surface file: a single ``mean2t`` (2m air temperature)
    ...
    """
    at_lat = np.linspace(47.0, 44.5, 6)  # descending
    at_lon = np.linspace(-85.5, -83.0, 6)  # ascending
    mean2t = xr.DataArray(
        np.full((len(at_lat), len(at_lon)), mean2t_value),
        coords={"latitude": at_lat, "longitude": at_lon},
        dims=["latitude", "longitude"],
        name="mean2t",
    )
    return xr.Dataset({"mean2t": mean2t})
...
# ---------------------------------------------------------------------------
# Happy path — temperature masking math
# ---------------------------------------------------------------------------
class TestTemperatureMaskingMath:
    """
    With a constant temperature field ``at`` over the lake mask, the math
    inside ``process_files`` reduces algebraically to::

        mean2t = (Σ(at · mask · area)) / Σ(mask · area)

    """

    def test_flxf_january_writes_expected_row(self, tmp_path, monkeypatch):
        """A January flxf file writes one DB row with the expected metadata and mean2t value."""
        # --- fakes ---
        fake_mask_ds = _build_fake_mask_dataset()
        mean2t_value = 273
        fake_flxf_ds = _build_fake_flxf_dataset(mean2t_value=mean2t_value)
        ...
        monkeypatch.setattr(
            "src.data_processor.nc.Dataset",
            lambda *a, **k: fake_mask_ds
        )
        monkeypatch.setattr(
            "src.data_processor.cfgrib.open_dataset",
            lambda *a, **k: fake_flxf_ds,
        )
        monkeypatch.setattr(
            "src.data_processor.CFSDatabase",
            lambda *a, **k: fake_db
        )
        ...
        # CFSv2 flxf naming:
        # flxf.<member>.<cfs_run>.<forecast_yyyymm>.<...>.grib.grb2
        flxf = "flxf.01.2024010100.202401.avrg.grib.grb2"
        (download_dir / flxf).write_text("")

        # Throw in an .idx file to confirm cleanup runs
        (download_dir / (flxf + ".idx")).write_text("")
        ...
        assert len(db_calls) == 1, (
            f"expected 1 db row, got {len(db_calls)}: {db_calls}"
        )
        ...
        assert surface == "lake"
        assert var == "air_temperature"

        # The fake dataset contains a constant mean2t value.
        expected = mean2t_value
    ...
    def test_idx_files_are_removed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("src.data_processor.cfgrib.open_dataset", lambda *a, **k: _build_fake_flxf_dataset())
        monkeypatch.setattr("src.data_processor.CFSDatabase", lambda *a, **k: MagicMock())
    ...
    def test_unknown_lake_prefix_is_silently_skipped(self, tmp_path, monkeypatch, capsys):
        ...
        inside the air temperature block, but the broad ``except Exception``
        ...
        download_dir = tmp_path / "downloads"
        download_dir.mkdir()
        (download_dir / "flxf.01.2024010100.202401.avrg.grib.grb2").write_text("")
        mask_path = tmp_path / "mask.nc"
        mask_path.write_text("")
        ...
        assert "ERROR processing temperature data" in captured.out
